      ...
      document.addEventListener('DOMContentLoaded', function() {
        ...
        const projectEvents = [
          {foreach from=$projekty item=projekt}
            {
              title: '{$projekt.nazwa|escape:'javascript'}',
              start: '{$projekt.data_rozpoczecia}',
              end: '{$projekt.data_zakonczenia}',
              allDay: true,
              color: '{if $projekt.status == "zakończony"}#2ecc71{else}#3498db{/if}',
              extendedProps: {
                type: 'project',
                id: {$projekt.id},
                status: '{$projekt.status|escape:'javascript'}',
                description: '{$projekt.opis|escape:'javascript'|truncate:200:"..."}'
              }
            }{if !$projekt@last},{/if}
          {/foreach}
        ];
        ...
        const taskEvents = [
          {foreach from=$zadania item=zadanie}
            {
              title: '{$zadanie.nazwa|escape:'javascript'}',
              start: '{$zadanie.termin}',
              allDay: true,
              color: '{if $zadanie.status == "zakończone"}#2ecc71{elseif $zadanie.status == "w trakcie"}#f39c12{else}#95a5a6{/if}',
              classNames: ['event-priority-{$zadanie.priorytet}'],
              extendedProps: {
                type: 'task',
                id: {$zadanie.id},
                status: '{$zadanie.status|escape:'javascript'}',
                priority: '{$zadanie.priorytet|escape:'javascript'}',
                project: '{$zadanie.projekt_nazwa|escape:'javascript'}',
                assigned: '{$zadanie.przypisane_do|escape:'javascript'}',
                description: '{$zadanie.opis|escape:'javascript'|truncate:200:"..."}',
                projectId: {$zadanie.projekt_id}
              }
            }{if !$zadanie@last},{/if}
          {/foreach}
        ];
